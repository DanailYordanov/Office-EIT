...
class BaseModelForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        for field in self.fields.values():
            if isinstance(field.widget, date_widget_classes):
                field.widget.attrs.update(
                    {'class': 'form-control date-picker', 'placeholder': field.label})
            elif isinstance(field.widget, text_widget_classes):
                field.widget.attrs.update(
                    {'class': 'form-control', 'placeholder': field.label})
            elif isinstance(field.widget, checkbox_widget_classes):
                field.widget.attrs.update(
                    {'class': 'form-check-input'})
            elif isinstance(field.widget, file_widget_classes):
                field.widget.attrs.update(
                    {'class': 'form-control'})
            elif isinstance(field, (forms.ModelChoiceField, forms.ModelMultipleChoiceField)):
                field.empty_label = 'Избери'


class CarModelForm(BaseModelForm):
    class Meta:
        model = models.Car
        fields = '__all__'


class ReminderModelForm(BaseModelForm):
    class Meta:
        model = models.Reminder
        fields = '__all__'


class ServiceModelForm(BaseModelForm):
    class Meta:
        model = models.Service
        fields = '__all__'


class ContractorsModelForm(BaseModelForm):
    class Meta:
